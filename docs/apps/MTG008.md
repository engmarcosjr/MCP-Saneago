# MTG008 - Consultar Remessas Capturadas

## Categoria
Financeiro, Tesouraria e Remessas

## Tipo
Leitura (Livre)

## O que faz
Permite visualizar e gerenciar informações relacionadas a consultar remessas capturadas.

## Campos da Tela Inicial
- **Sem Rotulo** (text): Editável
- **Grupo:** (text): Editável
- **Referência:** (date): Editável

## Botões Disponíveis
- **Consultar** (ID ZK: `zYRJo0`)
- **Cancelar** (ID ZK: `zYRJq0`)

## Exemplos de Intenção (Linguagem Natural)
- "Abrir a tela MTG008 (Consultar Remessas Capturadas)"
- "Acessar o aplicativo de consultar remessas capturadas"

## Roteiro de Operação
1. **Abrir a aplicação:** use a tool `saneago_abrir_e_inspecionar` com o código `MTG008`.
2. **Preencher os campos necessários** e clicar nos botões de consulta/ação conforme o fluxo desejado.

---
*Documento gerado automaticamente pelo MCP-Saneago em 16/07/2026 (Status: auto).*
