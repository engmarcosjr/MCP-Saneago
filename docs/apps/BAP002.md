# BAP002 - Contracheque

## Categoria
Recursos Humanos e Pessoal

## Tipo
Leitura (Livre)

## O que faz
Permite visualizar e gerenciar informações relacionadas a contracheque.

## Campos da Tela Inicial
- **Sem Rotulo** (text): Somente Leitura

## Botões Disponíveis
- **Consultar** (ID ZK: `kU4Uk0`)
- **Cancelar** (ID ZK: `kU4Ul0`)

## Exemplos de Intenção (Linguagem Natural)
- "Abrir a tela BAP002 (Contracheque)"
- "Acessar o aplicativo de contracheque"

## Roteiro de Operação
1. **Abrir a aplicação:** use a tool `saneago_abrir_e_inspecionar` com o código `BAP002`.
2. **Preencher os campos necessários** e clicar nos botões de consulta/ação conforme o fluxo desejado.

---
*Documento gerado automaticamente pelo MCP-Saneago em 15/07/2026 (Status: auto).*
